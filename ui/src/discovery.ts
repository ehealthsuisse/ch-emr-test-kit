...
import type { StructureDefinition } from "./fhir/types";

export interface ProfileRef {
  url: string;
  id?: string;
  name: string;
  type: string; // base resource type the profile constrains
}

export interface IgGroup {
  key: string; // url-safe key used in routes
  title: string;
  packageId?: string;
  version?: string;
  fallback?: boolean; // true for the catch-all "ungrouped" bucket
  profiles: ProfileRef[];
}

interface IgResource {
  ...
  version?: string;
  packageId?: string;
  definition?: { resource?: { reference?: { reference?: string } }[] };
}
...
function igBase(u: string): string {
...
function igTitleFromBase(base: string): string {
...
function profileFromSd(sd: StructureDefinition): ProfileRef {
  return {
    url: sd.url,
    id: sd.id,
...
export async function discoverIgs(): Promise<IgGroup[]> {
  const [igBundle, sdBundle] = await Promise.all([
    fhir.search<IgResource>("ImplementationGuide", { _count: 200 }).catch(() => undefined),
    fhir.search<StructureDefinition>("StructureDefinition", {
  ...
  const profiles = (sdBundle.entry || [])
    .map((e) => e.resource)
    .filter((sd): sd is StructureDefinition => !!sd && !!sd.url);

  const byId = new Map<string, StructureDefinition>();
  const byUrl = new Map<string, StructureDefinition>();
  for (const sd of profiles) {
    if (sd.id) byId.set(sd.id, sd);
    byUrl.set(sd.url, sd);
  }

  const claimed = new Set<string>();
  const groups: IgGroup[] = [];

  for (const e of igBundle?.entry || []) {
    const ig = e.resource;
    if (!ig) continue;
    const refs = ig.definition?.resource || [];
    const igProfiles: ProfileRef[] = [];
    for (const r of refs) {
      const ref = r.reference?.reference; // e.g. "StructureDefinition/abc" or a canonical
      if (!ref) continue;
      let sd: StructureDefinition | undefined;
      if (ref.startsWith("StructureDefinition/")) sd = byId.get(ref.slice("StructureDefinition/".length));
      else if (/^https?:/.test(ref)) sd = byUrl.get(ref);
  ...
  const byBase = new Map<string, ProfileRef[]>();
...
export function findProfile(groups: IgGroup[], groupKey: string, profileUrl: string): ProfileRef | undefined {
