...
import { DATATYPE_TEMPLATES, htmlInputType, PRIMITIVE_REGEX, PRIMITIVE_TYPES, } from "./datatypes";
import { buildTree, fixedOrPattern, maxNum } from "./tree";
const MAX_DEPTH = 8;
...
function coerce(code, raw) {
    if (code === "boolean")
        return raw === "true";
    if (["integer", "unsignedInt", "positiveInt"].includes(code))
        return parseInt(raw, 10);
    if (code === "decimal")
        return Number(raw);
    return raw; // string, code, dates, uri, integer64 (string), base64Binary, markdown
}
function primitiveWidget(code, ctx, initial) {
    let input;
    if (code === "boolean") {
...
function manualConceptWidget(code, node, ctx, initial) {
    if (code === "code")
        return primitiveWidget("code", ctx, initial);
...
function boundWidget(code, valueSetUrl, node, ctx, initial) {
    const holder = el("div", { class: "bound" }, [el("div", { class: "loading" }, ["loading value set…"])]);
    let inner;
    let current = initial;
    function useDropdown(contains) {
    ...
    function useManual(systemHint) {
        let initial = current;
        if (initial === undefined && systemHint) {
            // Seed the coding system so the user only needs to enter a code.
            if (code === "Coding")
                initial = { system: systemHint };
            else if (code === "CodeableConcept")
                initial = { coding: [{ system: systemHint }] };
        }
    ...
    async function fallbackFromCompose() {
        const def = await fhir.valueSetDefinition(valueSetUrl);
        const includes = def?.compose?.include || [];
        const concepts = [];
        for (const inc of includes) {
            for (const c of inc.concept || [])
                concepts.push({ system: inc.system, code: c.code, display: c.display });
        }
        if (concepts.length) {
            useDropdown(concepts);
            return;
        }
        const systems = [...new Set(includes.map((i) => i.system).filter((s) => !!s))];
    ...
    fhir
        ...
        if (contains.length)
            useDropdown(contains);
        else
            void fallbackFromCompose();
    })
...
function initialCode(code, v) {
    ...
    if (code === "Coding")
        return o.code;
    const coding = o.coding?.[0];
    return coding?.code;
}
// ---------------------------------------------------------------------------
// Type dispatch
// ---------------------------------------------------------------------------
function templateNodes(code, parentPath) {
    const t = DATATYPE_TEMPLATES[code] || [];
    return t.map((f) => ({
        name: f.name,
        children: [],
        ed: {
...
function objectWidget(children, ctx) {
...
function renderValueForType(code, node, ctx, initial) {
    ...
    if (PRIMITIVE_TYPES.has(code))
        return primitiveWidget(code, ctx, initial);
    if (node.children.length)
        return objectWidget(node.children, ctx);
    if (DATATYPE_TEMPLATES[code])
        return objectWidget(templateNodes(code, ctx.path), ctx);
    if (code === "Extension")
        return extensionWidget(node, ctx);
    return jsonWidget(initial);
}
function extensionWidget(node, ctx) {
...
async function loadExtension(profileUrl, ctx) {
...
function makeOccurrenceList(node, ctx, isChoice) {
    ...
    function sync() {
        ...
        for (const e of entries)
            e.rm.style.display = entries.length > min ? "" : "none";
    }
    function makeRow() {
    ...
    return {
        ...
        validate() {
            ...
            for (const e of entries)
                issues.push(...e.ctl.validate());
            return issues;
        },
    };
}
function renderOccurrence(node, ctx, isChoice) {
    if (ctx.depth > MAX_DEPTH)
        return jsonWidget();
    if (isChoice)
        return choiceWidget(node, ctx);
    const fp = fixedOrPattern(node.ed);
    if (fp?.locked)
        return lockedWidget(fp.value);
...
// ---------------------------------------------------------------------------
function cardBadge(node) {
    return `${node.ed.min ?? 0}..${node.ed.max ?? "1"}`;
}
function renderElement(name, nodes, ctx) {
    ...
    for (let i = 0; i < lists.length; i++) {
        ...
        const group = el("details", { class: `el-group${required ? " required" : ""}`, open: openByDefault }, [summary, lists[i].el]);
        wrap.append(group);
    }
    return {
        el: wrap,
            ...
            if (isChoice) {
                for (const list of lists) {
                    for (const v of list.getValues()) {
                        const pair = v;
                        if (pair?.propName !== undefined && pair.value !== undefined)
                            target[pair.propName] = pair.value;
                    }
                }
                return;
            }
        ...
        setFrom(source) {
            if (isChoice) {
                ...
                if (key !== undefined)
                    lists[0].setValues([{ propName: key, value: source[key] }]);
                return;
            }
...
function renderObject(children, ctx) {
    const groups = new Map();
    for (const c of children) {
        if (c.ed.max === "0")
            continue;
        if (ctx.depth === 0 && SKIP_TOP.has(c.name))
            continue;
...
export function buildProfileForm(sd, profileUrl) {
