import { el } from "./dom";
export function renderClientIssues(issues) {
...
export function renderOutcome(outcome) {
    ...
    box.append(el("h4", {}, [errors ? `Server validation: ${errors} error(s)` : "Server validation: passed"]));
    const ul = el("ul", {});
    for (const i of issues) {
        const loc = (i.expression || i.location || []).join(", ");
        ul.append(el("li", { class: `issue ${i.severity}` }, [
...
export function outcomeHasErrors(outcome) {
