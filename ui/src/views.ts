import { discoverIgs, findProfile, type IgGroup } from "./discovery";
import { clear, el } from "./dom";
import { fhir, FhirError } from "./fhir/client";
import type { Bundle, FhirResource } from "./fhir/types";
import { buildProfileForm, type ProfileForm } from "./form/engine";
import { outcomeHasErrors, renderClientIssues, renderOutcome } from "./validation";

let groupsCache: IgGroup[] | undefined;

async function getGroups(): Promise<IgGroup[]> {
  if (!groupsCache) groupsCache = await discoverIgs();
  return groupsCache;
}
...
function loading(msg = "Loading…"): HTMLElement {
  return el("div", { class: "loading" }, [msg]);
}

function errorBox(e: unknown): HTMLElement {
  const msg = e instanceof Error ? e.message : String(e);
  return el("div", { class: "error-box" }, [`Error: ${msg}`]);
}

function nativeTesterUrl(): string {
...
export async function renderHome(): Promise<void> {
  ...
  try {
    ...
    for (const g of groups) {
      grid.append(
        el("a", { class: "ig-card", href: `#/ig/${encodeURIComponent(g.key)}` }, [
          el("h3", {}, [g.title]),
          el("div", { class: "muted" }, [
...
export async function renderIg(params: Record<string, string>): Promise<void> {
  const root = app();
  clear(root);
  ...
  try {
    ...
    for (const p of g.profiles) {
      ...
      tbody.append(
        el("tr", {}, [el("td", {}, [link]), el("td", {}, [p.type]), el("td", { class: "url" }, [p.url])]),
      );
    }
    table.append(tbody);
    root.append(table);
...
export async function renderCrud(params: Record<string, string>): Promise<void> {
  const root = app();
  clear(root);
  ...
  try {
    ...
    if (!sd) throw new Error("StructureDefinition not retrievable");
  ...
  function resetForm(): void {
    editingId = undefined;
    form.setResource({ resourceType } as FhirResource);
  ...
  validateBtn.addEventListener("click", async () => {
    ...
    if (clientIssues.length) issuesArea.append(renderClientIssues(clientIssues));
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const saved = editingId
        ? await fhir.update<FhirResource>(resourceType, editingId, res)
        : await fhir.create<FhirResource>(resourceType, res);
      status.textContent = `Saved ${resourceType}/${saved.id}`;
      resetForm();
      await refreshList();
    } catch (e) {
      status.textContent = "";
      if (e instanceof FhirError && e.outcome) issuesArea.append(renderOutcome(e.outcome));
      else issuesArea.append(errorBox(e));
    }
  });
  ...
  async function loadIntoForm(id: string): Promise<void> {
    try {
      const res = await fhir.read<FhirResource>(resourceType, id);
  ...
  async function refreshList(): Promise<void> {
    ...
    try {
      const bundle: Bundle<FhirResource> = await fhir.search<FhirResource>(resourceType, {
        _profile: profile!.url,
        _count: 50,
        _sort: "-_lastUpdated",
      });
      clear(listBody);
      const entries = (bundle.entry || []).map((e) => e.resource).filter((r): r is FhirResource => !!r);
      if (!entries.length) {
      ...
      for (const r of entries) {
        ...
        const lastUpdated = ((r.meta as Record<string, unknown>) || {}).lastUpdated as string | undefined;
        ...
        delBtn.addEventListener("click", async () => {
          if (!confirm(`Delete ${resourceType}/${id}?`)) return;
          try {
